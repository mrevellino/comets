"""
Comparison between the obtained orbits when applying the single volatile 
sublimation model with n = 2 and n=2.3 for H2O sublimation 
"""
...
for i in range(2):
    ...
    # retrived the ground stations coordinates from the MPC
    ...
    formal_errors_dict[i] = pod_output.formal_errors
    final_parameters_dict[i] = pod_output.final_parameters
...
"""
state_history_pl = state_history_dict[0]
state_history_pl_2 = state_history_dict[1]

error_powerlaw = []

for key in state_history_pl.keys():
    error_powerlaw.append(list(np.array(state_history_pl[key]) - np.array(state_history_pl_2[key])))
...
y_error = []
z_error = []
covariance_pl = covariance_dict[0]
...
plt.tight_layout()
plt.savefig(f'n_co2_sensitivity_{config_number}.png')
plt.show()



diff_A1 = final_parameters_dict[1][-3] - final_parameters_dict[0][-3]
diff_A2 = final_parameters_dict[1][-2] - final_parameters_dict[0][-2]
diff_A3 = final_parameters_dict[1][-1] - final_parameters_dict[0][-1]

print(final_parameters_dict[1][-3], final_parameters_dict[0][-3])
print(final_parameters_dict[1][-2], final_parameters_dict[0][-2])
print(final_parameters_dict[1][-1], final_parameters_dict[0][-1])

print("Difference A1", diff_A1, "Formal error A1", formal_errors_dict[0][-3])
print("Difference A2", diff_A2, "Formal error A2", formal_errors_dict[0][-2])
print("Difference A3", diff_A3, "Formal error A3", formal_errors_dict[0][-1])