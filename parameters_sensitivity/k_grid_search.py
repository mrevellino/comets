"""
Analyze the effects of changing k and B on the residuals 
(fitting your own power law acceleration) and observe if changes in the 
values of k and B are significant 
"""
...
from comegs.accelerations import PowerLawAcceleration_kgrid
...
"""
Define the acceleration settings and set up the grid search for the values of B and k
"""
k_values = np.linspace(1e-3, 0.1, 20)
perform_search = True 
if perform_search:
    rms_ra = {}
    rms_dec = {}
    ...
    for k in k_values:
        ...
        nongrav_acc = PowerLawAcceleration_kgrid(A1, A2, A3, bodies, str(target_mpc_code), 
    ...
    # save results of the estimation in files for future access
    with open(f'acceleration_vs_k/rms_ra_{config_number}.pkl', 'wb') as f:
        pickle.dump(rms_ra, f)

    with open(f'acceleration_vs_k/rms_dec_{config_number}.pkl', 'wb') as f:
        pickle.dump(rms_dec, f)

    with open(f'acceleration_vs_k/rms_tot_{config_number}.pkl', 'wb') as f:
        pickle.dump(rms_tot, f)

"""
Read the saved results and plot them
"""
with open(f'acceleration_vs_k/rms_ra_{config_number}.pkl', "rb") as f:
    rms_ra = pickle.load(f)

with open(f'acceleration_vs_k/rms_dec_{config_number}.pkl', "rb") as f:
    rms_dec = pickle.load(f)


with open(f'acceleration_vs_k/rms_tot_{config_number}.pkl', "rb") as f:
    rms_tot = pickle.load(f)

# Unpack the dict
k = list(rms_ra.keys())[1:]
res_ra = list(rms_ra.values())[1:]
...
k = list(rms_dec.keys())[1:]
res_dec = list(rms_dec.values())[1:]
...
plt.tight_layout()
#plt.savefig(f'acceleration_vs_k/k_gridsearch_residuals_{config_number}.png')

key = list(rms_tot.keys())[1:]
res_tot = list(rms_tot.values())[1:]
...
plt.tight_layout()
#plt.savefig(f'acceleration_vs_k/k_gridsearch_total_residuals_{config_number}.png')
plt.show()